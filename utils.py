...
def sample_from_gaussian_1d(nn_out):
    return torch.normal(nn_out[:, 0, :, :], torch.exp(nn_out[:, 1, :, :])).unsqueeze(1).clamp(-1.0, 1.0)
...
def discretized_mix_logistic_prob(x, l):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)
    l = l.permute(0, 2, 3, 1)
...
def discretized_mix_logistic_prob_1d(x, l):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)
    l = l.permute(0, 2, 3, 1)
